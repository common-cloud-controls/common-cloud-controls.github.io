...
export type AppTheme = {
  ...
  radii: { lg: string; xl: string; pill: string };
  shadows: { surface: string; surfaceStrong: string };
  spacing: { md: string; lg: string; xl: string };
  typography: { body: string; mono: string };
};

const cccTheme: AppTheme = {
  colors: {
    background: "#020817",
    surface: "rgba(15, 23, 64, 0.65)",
    surfaceSubtle: "rgba(15, 23, 64, 0.4)",
    text: "#e2e8f0",
    textSubtle: "#94a3b8",
    accent: "#3b82f6",
  ...
  radii: { lg: "0.75rem", xl: "1rem", pill: "999px" },
  shadows: {
    surface: "0 8px 32px rgba(0,0,0,0.8), 0 0 0 1px rgba(59,130,246,0.15)",
    surfaceStrong: "0 12px 48px rgba(0,0,0,0.9), 0 0 20px rgba(59,130,246,0.35)"
  },
  spacing: { md: "1.25rem", lg: "2rem", xl: "2.5rem" },
  typography: {
...
type ThemeContextValue = { theme: AppTheme };
const ThemeContext = createContext<ThemeContextValue | undefined>(undefined);

export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const theme = useMemo<AppTheme>(() => cccTheme, []);
  ...
  const cssVariables: React.CSSProperties = {
    ...
    "--gf-accent-glow": "rgba(59, 130, 246, 0.4)",
    "--gf-glass-blur": "blur(12px)",
    "--gf-gradient-brand": "linear-gradient(180deg, #3b82f6, #6366f1, #1e40af)"
  } as React.CSSProperties;

  return (
    <ThemeContext.Provider value={value}>
      <div className="slam-theme" style={{ ...cssVariables, minHeight: "100%", background: theme.colors.background }}>
        {children}
      </div>
